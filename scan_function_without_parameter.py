...
def Scan():
    # call UpdateListAndMap(coords, True)
        ...
        if int(GetBearingIndegrees()) - 1 != int(initial_angle):
           ...
           elif object_detected_angle[-1] != 0:
               object_detected_angle.append(0)
            
        if int(GetBearingIndegrees()) == int(initial_angle):
            left_speed = 0.0
            right_speed = 0.0
...
while robot.step(TIME_STEP) != -1:
    if currentState == 0:
        initial_angle= GetBearingIndegrees()
        Scan()
